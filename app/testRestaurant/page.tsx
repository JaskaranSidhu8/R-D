...
const TestRestaurantPage = () => {
  ...
  const [allRestaurants, setAllRestaurants] = useState<
    { id: number; name: string; similarity: string }[]
  >([]);
  const groupId = 1;
    ...
    async function fetchBestRestaurant() {
      try {
        ...
        const response = await fetch(
          `/api/findBestRestaurant?groupId=${groupId}`,
        );
  ...
  return (
    <div>
      ...
      {bestRestaurant ? (
        <div>
          <p>
            <strong>Name:</strong> {bestRestaurant.name}
          </p>
          <p>
            <strong>Similarity:</strong> {bestRestaurant.similarity}
          </p>
        </div>
      ) : (
        ...
        <ul>
          {allRestaurants.map((restaurant) => (
            <li key={restaurant.id}>
              ...
              <p>
                <strong>Similarity:</strong> {restaurant.similarity}
              </p>
            </li>
          ))}
        </ul>
