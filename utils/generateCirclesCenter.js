export default function generateCircleCenters(centerLat, centerLng, circleRadius, totalRadius) {
    const earthRadius = 6371000; // Earth's radius in meters
    const circleCenters = [{ lat: centerLat, lng: centerLng }]; // Start with the center circle
  
    function generateLayerCircles(centerLat, centerLng, currentRadius) {
      ...
      for (let i = 0; i < numPoints; i++) {
        ...
        const newLat =
          centerLat + (currentRadius / earthRadius) * (180 / Math.PI) * Math.cos(angle);
        const newLng =
          centerLng +
    ...
    while (currentRadius <= totalRadius) {
      const newCircles = generateLayerCircles(centerLat, centerLng, currentRadius);
